...
## 3.局部变量是否可以返回指针到上层
可以,地址相同（第一次被问这个问题蒙了，以为是什么高端的问题，没反应过来，日常编程天天都在这样用）
...
输出
``` output
2020/08/01 17:48:27 1
2020/08/01 17:48:27 2
2020/08/01 17:48:27 3

2020/08/01 17:48:27 1
2020/08/01 17:48:27 200
2020/08/01 17:48:27 300
```
当range对象是array的时候，拷贝了array一整份值去进行操作；range对象是slice的时候，拷贝了slice的地址，然后对对应的值进行操作
...
func main() {
	...
	log.Println(*c2[0], *c2[1], *c2[2])
}

```
输出
``` output
2020/08/01 18:01:36 a地址:0xc000128090
2020/08/01 18:01:36 a地址:0xc000128098
2020/08/01 18:01:36 a地址:0xc0001280a0
2020/08/01 18:01:36 101 102 103
2020/08/01 18:01:36 b地址:0xc0001287d8
2020/08/01 18:01:36 b地址:0xc0001287d8
2020/08/01 18:01:36 b地址:0xc0001287d8
2020/08/01 18:01:36 [1 2 3]
2020/08/01 18:01:36 c1地址:0xc000128810
2020/08/01 18:01:36 c1地址:0xc000128810
2020/08/01 18:01:36 c1地址:0xc000128810
2020/08/01 18:01:36 3 3 3
```
for range中value只是同一个地址复制slice中元素的值，改变value的值不能改变原本slice的值,其他指针指向value的地址，会得到range的最后一个值；但是当slice中元素为指针时，value指向slice中对应元素的地址
...
输出
``` output
2020/08/01 18:08:38 0
2020/08/01 18:08:38 1
2020/08/01 18:08:38 2
2020/08/01 18:08:38 [1 2 3 1 2 3]
```
for range在开始前记录了slice长度，在for range中append增加slice长度并不会无限循环下去，达到增加前的长度时就会跳出循环